...
<template>
  <q-layout class="bg-white text-black" container style="min-width: 80vw;">
    <q-header class="bg-white text-black shadow-up-1">
      <q-toolbar class="">
        <q-btn flat round dense icon="fas fa-signature" color="purple" />
        <q-toolbar-title>For Signature</q-toolbar-title>
            ...
            >
              <td>
                <div v-if="props.row._id === hover" class="q-gutter-sm">
                  ...
                  />
                  <!-- <q-btn icon="delete" color="negative" size="sm" round @click="removeResponse(props.row)" /> -->
                </div>
              </td>
              <td>
              ...
              <td class="text-right" v-for="que in node.format" :key="que.field" :props="props">
                <div v-if="props.row[que.field]">
                  ...
                  <div v-else-if="que.type.value === 'Select'">
                    <span v-if="!que.isMultiple"> {{ props.row[que.field].value }} </span>
                  ...
                  <span v-else-if="que.type.value === 'digiSign'">
                    ...
                    {{ props.row[que.field].datetime ? $moment(props.row[que.field].datetime).format('YYYY-MM-DD') : props.row[que.field].assignedTo.label }}
                  </span>
                  <div v-else>
                    {{ props.row[que.field] }}
                  </div>
    ...
    <q-footer>
      <q-toolbar class="bg-white text-black shadow-1">
        ...
        <q-btn-toggle
          class="bg-grey-3"
          v-if="this.$root.user._id === node.owner"
          rounded
          unelevated
          v-model="tab"
          toggle-color="purple"
...
export default {
  ...
  methods: {
      ...
      if (this.userID) {
        ...
        this.node.format.map(que => {
          if (que.type.value && que.type.value === 'digiSign') {
            ...
            obj[`${que.field}.userId`] = { $ne: this.userID }
            params.query.$or.push(obj)
            config.channels.push({
              prop: `${que.field}`,
              value: (val, message) => {
                return val.assignedTo.value === this.userID &&
    ...
    async exportCSV () {
      ...
      data.map(row => {
        ...
        this.node.format.map(que => {
          if (que.type.value === 'Select') {
            rec[que.field] = !que.isMultiple ? row[que.field].value : row[que.field].map(v => v.value).join(', ')
          } else if (que.type.value === 'digiSign') {
            rec[`${que.field} - Signer`] = row[que.field].displayName
            rec[`${que.field} - date`] = this.$moment(row[que.field].datetime).format('YYYY-MM-DD')
      ...
      this.$arrayToCSV(arr, this.node.label + ' - ' + Date.now() + '.csv')
    },
    openResponse (res) {
      this.response = res
