      ...
      >
        <q-card-section style="max-height: 56px;" class="row items-center no-wrap">
            <div class="col text-weight-regular limited-text" :class="{
              ...
              {{ folder.label }}
              <q-tooltip v-if="folder.label.length > 25">
                {{ folder.label }}
              </q-tooltip>
            </div>
...
export default {
  ...
  methods: {
    ...
    checkSecurity (sec) {
      let isRootUser = (user) => user.value._id === this.$root.user._id
      return sec.public ||
        this.$lodash.findIndex(sec.viewers, isRootUser) > -1 ||
        this.$lodash.findIndex(sec.uploaders, isRootUser) > -1 ||
        this.$lodash.findIndex(sec.modifiers, isRootUser) > -1 ||
        this.$lodash.findIndex(sec.removers, isRootUser) > -1 ||
        this.$lodash.findIndex(sec.folderCreators, isRootUser) > -1 ||
        this.$lodash.findIndex(sec.approvers, isRootUser) > -1
    },
    getFoldersAndFiles () {
        ...
        channels: [
          {
            value: (security, folder) => { // Channel name
              console.log('checkSecurity', this.checkSecurity(security), folder.parentFolder === this.$route.params._id)
              return this.checkSecurity(security)
            },
